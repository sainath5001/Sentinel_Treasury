...
export function T3TrustBanner() {
  ...
  if (isLoading) {
    return (
      <div className="mb-6 flex items-center gap-3 rounded-xl border border-violet-500/15 bg-black/50 px-4 py-3">
  ...
  return (
    <div className="mb-6 rounded-xl border border-violet-500/20 bg-black/50 p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">
              {data?.delegations.filter((d) => d.active).length ?? 0} active delegations · Adapter:{" "}
              {data?.adapter ?? "—"}
            </p>
            {data?.ownerDid && (
              <p className="mt-1 flex items-center gap-1 font-mono text-[10px] text-violet-300/80">
